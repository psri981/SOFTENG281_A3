...
public class SetOfStrings {
    ...
    SetOfStrings out = new SetOfStrings();
        ...
        if (other.isMember(elThis)){
				  out.insertElement(elThis);
        }
		}
		return out;
  }
    ...
    SetOfStrings out = new SetOfStrings();
    for (String elThis : this.elements) {
        if (!other.isMember(elThis)){
				  out.insertElement(elThis);
        }
		}
  ...
  // continues adding elements until at empty array index
  private void addAllElements(SetOfStrings set) {
		Enumeration<String> enum1 = set.elements();
